...
export class GridStack {
  ...
  public static addGrid(
    ...
    if (!parentIsGrid || GridStack.addRemoveCB) {
      if (GridStack.addRemoveCB) {
        ...
        doc.body.innerHTML = `<div class="grid-stack ${
          opt.class || ""
        }"></div>`;
  ...
  public load(
    ...
    const haveCoord = layout.some(
      (w) => w.x !== undefined || w.y !== undefined
    );
    let items = haveCoord
      ? Utils.sort(layout, -1, this._prevColumn || this.getColumn())
      : layout;
    ...
    if (
      ...
      items.some((n) => (n.x || 0) + n.w > (this.opts.column as number))
    ) {
    ...
    if (typeof addRemove === "function")
      GridStack.addRemoveCB = addRemove as AddRemoveFcn;
    ...
    items.forEach((w) => {
      let item =
        w.id !== undefined ? copyNodes.find((n) => n.id === w.id) : undefined;
      if (item) {
  ...
  public resizeToContent(els: GridStackElement) {
    GridStack.getElements(els).forEach((el) => {
      ...
      if (!child) {
        console.log(
          `Error: resizeToContent() '${GridStack.resizeToContentParent}'.firstElementChild is null, make sure to have a div like container. Skipping sizing.`
        );
        return;
      }
  ...
  protected _updateStyles(forceUpdate = false, maxH?: number): GridStack {
    ...
    if (!this._styles) {
      ...
      Utils.addCSSRule(
        this._styles,
        prefix,
        `height: ${cellHeight}${cellHeightUnit}`
      );
      // content margins
  ...
  public static setupDragIn(
    ...
    let els: HTMLElement[] =
      typeof dragIn === "string" ? Utils.getElements(dragIn, root) : dragIn;
    if (els.length)
      els?.forEach((el) => {
  ...
  public enableResize(doEnable: boolean, recurse = true): GridStack {
    ...
    doEnable
      ...
      : (this.opts.disableResize = true); // FIRST before we update children as grid overrides #1658
    this.engine.nodes.forEach((n) => {
      this._prepareDragDropByNode(n);
  ...
  private _itemRemoving(el: GridItemHTMLElement, remove: boolean) {
    ...
    if (
      !node ||
      !node.grid ||
      el.classList.contains(this.opts.removableOptions.decline)
    )
      return;
